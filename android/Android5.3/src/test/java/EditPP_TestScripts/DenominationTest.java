...
public class DenominationTest {
    ...
    public void subCaste_SearchTest() throws Throwable {
		...
		Thread.sleep(2000);
		String denomination_pp=g.getDenomination_pp().getText();
		System.out.println("Denomination: "+denomination_pp);
		String[] pp_Denomination=denomination_pp.split(",");
		...
		//=============Navigating Back From EPP========
				...
				if(lst.size()>0) {
					...
					for( int i=1;i<=2;i++) {
					    ...
					    String vp_Demoninaton=g.getDenomination_pp().getText();
					    System.out.println("Denomination in VP: "+vp_Demoninaton);
					    
					    System.out.println("Verifying vp Denomination with pp............");
					    if(!denomination_pp.equalsIgnoreCase("Any")){
					    	 boolean statusDeno=false;
					    	 for(int j=0;j<=pp_Denomination.length-1;j++){
					    		 if(vp_Demoninaton.equalsIgnoreCase(pp_Denomination[j])){
					    			 statusDeno=true;
								    	System.out.println("View profile Sect: "+vp_Demoninaton+"  is matching with pp "+pp_Denomination[j]);
								    }
					    		 
					    	 }
							    
							    asrt.assertTrue(statusDeno, "View profile: " +oppMatriId+" Denomination is not as per PP values");
					    }
